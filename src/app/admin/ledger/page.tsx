...
export default async function LedgerPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; edit?: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="space-y-2"><p className="text-sm font-medium text-navy-deep">To correct a manual submission, select the blue <span className="rounded bg-primary px-2 py-1 text-xs text-primary-foreground">Edit entry</span> button beside its date.</p><div className="overflow-x-auto rounded-xl border bg-card"><table className="w-full text-sm"><thead><tr className="border-b bg-muted/40 text-left">{["Date","Edit","Type","Category","Description","Income","Shipping","Tax","Expense","Net","Reference"].map(header=><th key={header} className="px-3 py-3">{header}</th>)}</tr></thead><tbody>{rows.length ? rows.map(row=><tr key={row.id} className="border-b"><td className="whitespace-nowrap px-3 py-3">{dateText(row.date)}</td><td className="whitespace-nowrap px-3 py-3">{row.manualId ? <Link className="inline-flex h-8 items-center rounded-md bg-primary px-3 font-medium text-primary-foreground hover:bg-primary/90" href={`/admin/ledger?from=${fromText}&to=${toText}&edit=${row.manualId}#edit-entry`}>Edit entry</Link> : <span className="text-xs text-muted-foreground">From order</span>}</td><td className="px-3 py-3">{row.type}</td><td className="px-3 py-3">{row.category}</td><td className="min-w-80 px-3 py-3">{row.description}</td><td className="px-3 py-3 text-right">{formatCurrency(row.merchandise)}</td><td className="px-3 py-3 text-right">{formatCurrency(row.shipping)}</td><td className="px-3 py-3 text-right">{formatCurrency(row.tax)}</td><td className="px-3 py-3 text-right">{formatCurrency(row.expense)}</td><td className="px-3 py-3 text-right font-medium">{formatCurrency(row.total)}</td><td className="px-3 py-3">{row.reference}</td></tr>) : <tr><td colSpan={11} className="px-4 py-8 text-center text-muted-foreground">No ledger entries in this date range.</td></tr>}</tbody></table></div></div>
