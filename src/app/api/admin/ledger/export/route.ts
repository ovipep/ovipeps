...
export async function GET(request: Request) {
  ...
  const lines = [["Date","Type","Category","Description","Merchandise Income","Shipping Income","Tax","Expense","Net","Payment Method","Reference","Notes"].map(csv).join(","), ...rows.map(r=>[r.date.toISOString().slice(0,10),r.type,r.category,r.description,r.merchandise,r.shipping,r.tax,r.expense,r.total,r.paymentMethod,r.reference,r.notes].map(csv).join(","))];
