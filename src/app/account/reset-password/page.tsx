...
function ResetForm() {
  ...
  return <Card><CardHeader><CardTitle>Choose a new password</CardTitle></CardHeader><CardContent>{status === "success" ? <p className="text-sm">Password updated. <Link href="/account/login" className="font-semibold text-sky hover:underline">Sign in</Link>.</p> : <form onSubmit={submit} className="space-y-4"><Input name="password" label="New password" type="password" minLength={12} maxLength={128} required /><p className="text-xs text-muted-foreground">Use at least 12 characters and do not reuse a password from another site.</p><Input name="confirm" label="Confirm password" type="password" minLength={12} maxLength={128} required />{status === "error" && <p className="text-sm text-error">{error}</p>}<Button type="submit" className="w-full">Update password</Button></form>}</CardContent></Card>;
}
