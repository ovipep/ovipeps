...
export function RestockSignup({ products }: { products: ProductOption[] }) {
  ...
  return (
    <section className="mb-8 overflow-hidden rounded-2xl border border-sky/25 bg-gradient-to-r from-sky/10 via-cyan/10 to-white shadow-sm">
      <button
        type="button"
      ...
      >
        <span className="flex items-center gap-3">
          <span className="rounded-xl bg-sky p-2.5 text-white"><Bell className="h-5 w-5" /></span>
        ...
        <ChevronDown className={`h-5 w-5 shrink-0 text-sky transition-transform ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <form onSubmit={submit} className="border-t border-sky/20 px-5 py-5">
            ...
            <div className="text-sm font-semibold text-navy-deep">
              ...
              <details className="group relative mt-2">
                ...
                <div className="mt-2 max-h-64 space-y-1 overflow-y-auto rounded-xl border border-border bg-white p-2 shadow-lg lg:absolute lg:z-20 lg:w-full">
                  <label className="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2.5 font-semibold hover:bg-sky/5">
                  ...
                  {products.map((product) => (
                    <label key={product.id} className="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2.5 font-normal hover:bg-sky/5">
          ...
          <fieldset className="mt-5">
            ...
            <div className="mt-2 grid gap-3 sm:grid-cols-2">
              <label className={`cursor-pointer rounded-xl border p-4 ${notificationType === "ONE_TIME" ? "border-sky bg-sky/5" : "border-border bg-white"}`}>
              ...
              <label className={`cursor-pointer rounded-xl border p-4 ${notificationType === "ONGOING" ? "border-sky bg-sky/5" : "border-border bg-white"}`}>
          ...
          <Button className="mt-5" type="submit" disabled={busy || (!allInventory && selected.length === 0)}>
