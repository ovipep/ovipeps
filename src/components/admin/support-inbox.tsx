...
export function SupportInbox({ initialConversations }: { initialConversations: Conversation[] }) {
  ...
  return <div className="space-y-4">{items.length === 0 ? <div className="rounded-xl border bg-white p-8 text-center text-sm text-muted-foreground">No customer messages yet.</div> : items.map(item => <article key={item.id} className="rounded-xl border bg-white p-5 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-bold text-navy-deep">{item.name} <span className="font-normal text-muted-foreground">({item.email})</span></p><p className="mt-1 text-xs text-muted-foreground">{item.reference} · {new Date(item.createdAt).toLocaleString()}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.status === "RESPONDED" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{item.status === "RESPONDED" ? "Responded" : "Awaiting response"}</span></div>
