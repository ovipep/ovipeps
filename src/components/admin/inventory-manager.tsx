...
export function InventoryManager({ products }: { products: Product[] }) {
  ...
  return <div className="space-y-5">
    ...
    <form className="grid gap-3 rounded-xl border border-dashed border-primary/40 bg-primary/5 p-4 sm:grid-cols-[2fr_1fr_auto]" onSubmit={(event) => {
      event.preventDefault(); const data = new FormData(event.currentTarget);
      void send("/api/admin/products", "POST", { name: data.get("name"), researchCategory: data.get("category") }, "new-product"); event.currentTarget.reset();
    }}>
      <Input name="name" placeholder="New product name" required />
      <Input name="category" placeholder="Research category" />
      <Button disabled={busy === "new-product"}><Plus className="h-4 w-4" /> Add product</Button>
    </form>
    ...
    {products.map((product) => <section key={product.id} className="overflow-hidden rounded-xl border border-border bg-card">
      ...
      <form className="grid gap-2 border-t border-border bg-muted/20 p-3 sm:grid-cols-5" onSubmit={(event) => {
        event.preventDefault(); const data = new FormData(event.currentTarget);
        void send(`/api/admin/products/${product.id}/variants`, "POST", { size: data.get("size"), sku: data.get("sku"), price: Number(data.get("price")), stockQuantity: Number(data.get("stock")) }, `new-${product.id}`); event.currentTarget.reset();
      }}>
        <Input name="size" placeholder="Vial size, e.g. 10mg" required /><Input name="sku" placeholder="Unique SKU" required /><Input name="price" type="number" min="0" step="0.01" placeholder="Price" required /><Input name="stock" type="number" min="0" step="1" placeholder="Inventory" required /><Button variant="outline" disabled={busy === `new-${product.id}`}><Plus className="h-4 w-4" /> Add vial size</Button>
      </form>
    </section>)}
  </div>;
}
